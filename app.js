...
function tampilkanProfil(peserta) {
  ...
  if (qc) { qc.innerHTML=''; QRCode.toCanvas(peserta.id,{width:180,margin:1},(e,c)=>{ if(!e) qc.appendChild(c); }); }
...
function generateAdminQR() {
  ...
  c.innerHTML=''; QRCode.toCanvas(url,{width:220,margin:2},(e,cv)=>{ if(!e) c.appendChild(cv); });
}

function printQR() {
  const canvas=document.querySelector('#admin-qr-pendaftaran canvas'); if(!canvas) return;
