...
const CONFIG = {
  ...
  imgbbApiKey : "b404cd643c3887bfc90b4b80106d4daf",
...
function onTimerUpdate() {
  ...
  // (hanya sekali, dan hanya jika peserta sudah login)
  if (page==='peserta' && STATE.eventStatus==='ended' && !STATE._voteAutoOpened && STATE.pesertaAktif) {
    STATE._voteAutoOpened = true;
    setTimeout(() => showVoting(), 600);  // delay sedikit biar mulus
  }
  // Reset penanda kalau event dimulai lagi (running)
...
/**
 * ── ADMIN: Stop permainan lebih awal ──
 * Akhiri event seketika (status 'ended') tanpa hapus data.
 * Efek sama seperti waktu habis: scan mati, leaderboard tetap tampil, voting kebuka.
 */
async function stopEvent() {
  ...
  // Set endAt ke waktu sekarang agar dianggap berakhir, status langsung 'ended'
  await db.collection('event').doc('status').set({
...
function updateAdminTimerUI() {
  ...
  if (STATE.eventStatus==='running') {
    ...
    if (btnStop)  btnStop.style.display='inline-block';   // tombol stop hanya saat berjalan
...
function tampilkanProfil(peserta) {
  ...
  // Kalau peserta login saat event SUDAH selesai, langsung tampilkan voting
